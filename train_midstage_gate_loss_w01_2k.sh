...
# 这次实验核心：显式监督 mid-stage gate
USE_MIDSTAGE_GATE_LOSS="True"
MIDSTAGE_GATE_LOSS_WEIGHT="0.01"

########################################
# Output
########################################
MIDSTAGE_OUTPUT_DIR="/home/wangchengjun/huangziyi/reseg/output/tgi/midstage_gate_loss_w01_2k"
...
echo "[INFO] USE_MIDSTAGE_GATE_LOSS=${USE_MIDSTAGE_GATE_LOSS}"
echo "[INFO] MIDSTAGE_GATE_LOSS_WEIGHT=${MIDSTAGE_GATE_LOSS_WEIGHT}"
...
if ! python segearth_r2/train/train.py --help 2>/dev/null | grep -q "use_midstage_gate_loss"; then
  echo "[ERROR] train.py does not expose --use_midstage_gate_loss."
  echo "        Please apply the gate loss code patch first."
  exit 1
fi

if ! python segearth_r2/train/train.py --help 2>/dev/null | grep -q "midstage_gate_loss_weight"; then
  echo "[ERROR] train.py does not expose --midstage_gate_loss_weight."
  echo "        Please apply the gate loss code patch first."
  exit 1
fi
...
########################################
echo "========================================"
echo "[1/5] Training mid-stage gate loss experiment: w=0.01, 2k"
echo "========================================"

export WANDB_NAME="tgi-midstage-gate-loss-w01-2k"
...
########################################
echo "========================================"
echo "[3/5] Merge selected checkpoint"
echo "========================================"
...
echo "========================================"
echo "[5/5] DONE"
echo "Output dir       : ${MIDSTAGE_OUTPUT_DIR}"
echo "Selected ckpt    : ${MIDSTAGE_BEST_CHECKPOINT}"
echo "Merged model     : ${MIDSTAGE_MERGED_DIR}"
echo "Test output      : ${MIDSTAGE_TEST_OUTPUT_DIR}"
echo "========================================"