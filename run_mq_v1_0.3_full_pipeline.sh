#!/usr/bin/env bash
# Mask Quality v1（OHEM-BCE）：train → best checkpoint → merge/export → verify → eval → metrics
#
# 默认一次执行顺序跑完 7w + 28w 全流程（各含 train→merge→eval→metrics）：
#   bash run_mq_v1_full_pipeline.sh
# 只跑一种：
#   MQ_PROFILE=7w bash run_mq_v1_full_pipeline.sh
#   MQ_PROFILE=28w bash run_mq_v1_full_pipeline.sh
#   bash run_mq_v1_full_pipeline.sh 7w
#   bash run_mq_v1_full_pipeline.sh 28w
# 显式 MAX_STEPS / SAVE_STEPS / OUTPUT_DIR 仅适用于单档位（both 模式下勿设）。
#
# 与 QMC 脚本相同风格的可选环境变量：
...
#   MQ_PROFILE=7w|28w|both    — both=顺序跑 7w+28w（默认）；7w/28w=只跑一种
...
MASK_CONFIG="${MASK_CONFIG:-segearth_r2/model/mask_decoder/mask_config/maskformer2_swin_base_384_bs16_50ep_mq_v1_ohem.yaml}"
...
OHEM_RATIO_TAG="${OHEM_RATIO_TAG:-0.3}"
...
resolve_mq_run_profiles () {
  local mode="${MQ_PROFILE:-both}"
  if [[ $# -ge 1 ]]; then
    case "${1}" in
      7w|28w|both|all)
        mode="${1}"
  ...
  case "${mode}" in
    both|all)
      MQ_RUN_PROFILES=(7w 28w)
      ;;
    7w|28w)
      MQ_RUN_PROFILES=("${mode}")
      ;;
    *)
      die "Unknown MQ_PROFILE=${mode} (use 7w, 28w, or both)"
      ;;
  esac
  if [[ ${#MQ_RUN_PROFILES[@]} -gt 1 ]]; then
    if [[ -n "${_USER_MAX_STEPS}" || -n "${_USER_SAVE_STEPS}" ]]; then
      die "MQ_PROFILE=both cannot combine with preset MAX_STEPS/SAVE_STEPS. Unset them or run one profile at a time."
    fi
    if [[ -n "${_USER_OUTPUT_DIR}" || -n "${_USER_MERGED_DIR}" || -n "${_USER_TEST_OUTPUT_DIR}" || -n "${_USER_RUN_TAG}" ]]; then
      die "MQ_PROFILE=both cannot combine with preset OUTPUT_DIR/MERGED_DIR/TEST_OUTPUT_DIR/RUN_TAG. Unset them or run one profile at a time."
    fi
  fi
}

configure_mq_profile () {
  local profile="$1"
  MQ_PROFILE="${profile}"
  case "${MQ_PROFILE}" in
    7w)
      ...
      ;;
    28w)
      MAX_STEPS="${_USER_MAX_STEPS:-280000}"
      SAVE_STEPS="${_USER_SAVE_STEPS:-5000}"
      ;;
    *)
      die "configure_mq_profile: bad profile ${MQ_PROFILE}"
      ;;
  esac
  if [[ -n "${_USER_RUN_TAG}" ]]; then
    RUN_TAG="${_USER_RUN_TAG}"
  else
    RUN_TAG="mq_v1_ohem_ratio${OHEM_RATIO_TAG}_siglip2_${MQ_PROFILE}"
  fi
...
########################################
# Global preflight + 顺序跑 7w → 28w（或单档位）
########################################
resolve_mq_run_profiles "$@"
...
echo "========================================"
echo "[PLAN] MQ-v1 will run profile(s): ${MQ_RUN_PROFILES[*]}"
echo "========================================"

for _mq_prof in "${MQ_RUN_PROFILES[@]}"; do
  echo ""
  echo "################################################################"
  echo "# START MQ_PROFILE=${_mq_prof}"
  echo "################################################################"
  configure_mq_profile "${_mq_prof}"
  run_mq_v1_pipeline_once
  echo "################################################################"
  echo "# FINISHED MQ_PROFILE=${_mq_prof}"
  echo "################################################################"
  echo ""
done

echo "========================================"
echo "ALL DONE MQ-v1 OHEM-BCE (${MQ_RUN_PROFILES[*]})"
echo "========================================"
