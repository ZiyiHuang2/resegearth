...
export WANDB_PROJECT=segearth-seg
...
# Output root
########################################
EXP_ROOT="/home/wangchengjun/huangziyi/reseg/output/segqv2"

run_one () {
  local exp_name="$1"
  local master_port="$2"
  local query_score_loss_weight="$3"
  local use_query_margin_loss="$4"
  ...
  echo "query_score_loss_weight=${query_score_loss_weight}"
  echo "use_query_margin_loss=${use_query_margin_loss}"
  echo "========================================"
  ...
  deepspeed --master_port="${master_port}" --include="${GPU_SLOT}" segearth_r2/train/train.py \
    ...
    --use_query_score_supervision True \
    --use_query_margin_loss "${use_query_margin_loss}" \
    --query_score_loss_weight "${query_score_loss_weight}" \
    --query_div_loss_weight 0.05 \
    --query_margin_loss_weight 0.1 \
    --inference_query_select_mode soft
...
# 1) 主实验：score=0.05, margin=on
run_one "segqv2_k4_score005_margin_on_soft" 29501 0.05 True

# 2) Ablation 1：score=0.0, margin=on
run_one "segqv2_k4_score000_margin_on_soft" 29502 0.0 True

# 3) Ablation 2：score=0.05, margin=off
run_one "segqv2_k4_score005_margin_off_soft" 29503 0.05 False

echo "========================================"
echo "All seg_query_adapter_v2 experiments completed."
echo "========================================"