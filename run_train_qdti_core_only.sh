...
########################################
# Init mode
#   cold   : Mipha cold start, predictor_init builds qdti_core
#   merged : resume from merged baseline without qdti_core (must allow random init)
########################################
INIT_MODE="cold"
...
# Output
########################################
OUTPUT_DIR="/root/rivermind-data/huangziyi/reseg/output/tgi/qdti-8w"
...
########################################
# QDTI-Core (Stage 2.6 v3.1, core-only)
########################################
...
merge_ckpt () {
  ...
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python segearth_r2/train/merge_lora_weights_and_save_hf_model.py \
    ...
    --use_qdti_mask_feedback "${USE_QDTI_MASK_FEEDBACK}" \
    --use_qdti_rank_loss "${USE_QDTI_RANK_LOSS}" \
    --use_qdti_neg_loss "${USE_QDTI_NEG_LOSS}" \
    --use_qdti_div_loss "${USE_QDTI_DIV_LOSS}" \
    --qdti_gate_init "${QDTI_GATE_INIT}" \
    --qdti_warmup_steps "${QDTI_WARMUP_STEPS}" \
    --allow_random_qdti_init "${ALLOW_RANDOM_QDTI_INIT}" \
    --decoder_attn_bias_max_abs "${DECODER_ATTN_BIAS_MAX_ABS}" \
    --decoder_attn_bias_apply_layers "${DECODER_ATTN_BIAS_APPLY_LAYERS}"
}

check_merged_qdti_config () {
  local merged_dir="$1"
...
use_qdti = bool(cfg.get("use_query_aware_decoder_bias", False))
print(f"[INFO] use_query_aware_decoder_bias={use_qdti}")
if not use_qdti:
    print("[ERROR] merged config missing use_query_aware_decoder_bias=True")
    sys.exit(1)
...
for name in ("model.safetensors.index.json",):
    ...
    if p.is_file():
        wm = json.load(open(p, encoding="utf-8")).get("weight_map", {})
        keys = [k for k in wm if "qdti_core" in k]
        break
single = Path("${merged_dir}") / "model.safetensors"
if not keys and single.is_file():
    from safetensors import safe_open
    with safe_open(str(single), framework="pt") as f:
        keys = [k for k in f.keys() if "qdti_core" in k]
binf = Path("${merged_dir}") / "pytorch_model.bin"
if not keys and binf.is_file():
    import torch
    sd = torch.load(binf, map_location="cpu")
    keys = [k for k in sd if "qdti_core" in k]

print(f"[INFO] merged qdti_core weight keys={len(keys)}")
if keys:
    print("[OK] sample:", keys[:3])
else:
    print("[WARN] no qdti_core keys in merged weights; eval may fail-fast")
PY
}
...
echo "[INFO] ALLOW_RANDOM_QDTI_INIT=${ALLOW_RANDOM_QDTI_INIT}"
echo "[INFO] USE_QUERY_AWARE_DECODER_BIAS=${USE_QUERY_AWARE_DECODER_BIAS}"
...
########################################
echo "========================================"
echo "[1/6] Training QDTI-Core only"
echo "========================================"
...
########################################
# 4) Check merged config + qdti weights
########################################
echo "========================================"
echo "[4/6] Check merged QDTI config/weights"
echo "========================================"
...
check_merged_qdti_config "${MERGED_DIR}"
