#!/usr/bin/env bash
# 7w：public_semantic_v2 + --concept_refaware_prior；训练/合并/评测流程与 match_strict 7w 脚本对齐。
set -euo pipefail
...
GPU_ID="${GPU_ID:-1}"
MASTER_PORT="${MASTER_PORT:-29112}"

REPO_DIR="/home/wangchengjun/huangziyi/reseg/resegearth+source"
RESEG_ROOT="/home/wangchengjun/huangziyi/reseg"
cd "${REPO_DIR}"

MODEL_NAME_OR_PATH="${MODEL_NAME_OR_PATH:-/home/wangchengjun/huangziyi/reseg/output/standard-base-siglip11/merged_model}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${RESEG_ROOT}/output/source/rrsisd_public_semantic_v2_refaware_exclusion_only_7w}"
MERGED_DIR="${OUTPUT_DIR}/merged_model"
TEST_OUTPUT_DIR="${OUTPUT_DIR}/test_results"
export WANDB_NAME="rrsisd_public_semantic_v2_refaware_exclusion_only_7w"

########################################
# Helpers（与 run_train_public_semantic_v2_match_strict_7w.sh 一致）
########################################
...
if ! python segearth_r2/train/train.py --help 2>&1 | grep -q "concept_refaware_prior"; then
  echo "[ERROR] train.py --help 未包含 concept_refaware_prior，请在正确 conda 环境中运行。"
  exit 1
fi
...
########################################
echo "========================================"
echo "[1/6] Training rrsisd_public_semantic_v2_refaware_exclusion_only_7w"
echo "========================================"
...
########################################
echo "========================================"
echo "[2/6] Select checkpoint (refaware_exclusion_only)"
echo "========================================"
...
if [[ ! -f "${MERGED_DIR}/config.json" ]]; then
  echo "[ERROR] merged config.json missing under ${MERGED_DIR}"
  exit 1
fi
...
########################################
echo "========================================"
echo "[5/6] Eval + metrics (refaware_exclusion_only)"
echo "========================================"

eval_model "${MERGED_DIR}" "${TEST_OUTPUT_DIR}"
run_eval_metrics "${TEST_OUTPUT_DIR}" "rrsisd_public_semantic_v2_refaware_exclusion_only_7w"

########################################
# [6/6] Done
########################################
echo "========================================"
echo "[6/6] DONE rrsisd_public_semantic_v2_refaware_exclusion_only_7w"
echo "========================================"
