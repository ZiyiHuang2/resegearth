#!/usr/bin/env bash
# SET++ only ablation: decoder-side set consistency, TG-Swin disabled.
set -euo pipefail

export NCCL_P2P_DISABLE=1
export NCCL_IB_DISABLE=1

deepspeed --master_port="${MASTER_PORT:-29502}" --include "${GPU_SLOT:-localhost:0}" segearth_r2/train/train.py \
    ...
    --output_dir "${OUTPUT_DIR:-output/ablation/setpp}" \
