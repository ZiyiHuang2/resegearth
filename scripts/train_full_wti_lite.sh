#!/usr/bin/env bash
# Full + WTI-lite gate ablation (optional; not default Full).
set -euo pipefail

export NCCL_P2P_DISABLE=1
export NCCL_IB_DISABLE=1

deepspeed --master_port="${MASTER_PORT:-29505}" --include "${GPU_SLOT:-localhost:0}" segearth_r2/train/train.py \
    ...
    --output_dir "${OUTPUT_DIR:-output/ablation/full_wti_lite}" \
    ...
    --mask_config 'segearth_r2/model/mask_decoder/mask_config/maskformer2_ablation_full_wti_lite.yaml' \
    --data_ratio '1' \
