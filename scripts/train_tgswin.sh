#!/usr/bin/env bash
# TG-Swin only ablation: encoder-side target-conditioned grounding, SET++ off.
set -euo pipefail

export NCCL_P2P_DISABLE=1
export NCCL_IB_DISABLE=1

deepspeed --master_port="${MASTER_PORT:-29503}" --include "${GPU_SLOT:-localhost:0}" segearth_r2/train/train.py \
    ...
    --output_dir "${OUTPUT_DIR:-output/ablation/tgswin}" \
    ...
    --mask_config 'segearth_r2/model/mask_decoder/mask_config/maskformer2_ablation_tgswin_only.yaml' \
    --data_ratio '1' \
