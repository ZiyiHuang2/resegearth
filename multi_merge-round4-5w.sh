...
export WANDB_PROJECT=segearth-itaa-round
...
run_one () {
  ...
  echo "seg_hidden_layer=${seg_hidden_layer}"
  echo "========================================"
...
PY
  ...
  if [[ -z "${BEST_CHECKPOINT}" ]]; then
    echo "[ERROR] best_model_checkpoint not found"
    exit 1
  fi
...
# 先跑两组最有信息量的
run_one "coarse_refine_layer2_w005_unfreezePD-5w" 29541 0.05 False -2
run_one "coarse_refine_base_w005_unfreezePD-5w"   29542 0.05 False -1