#!/usr/bin/env bash
# =============================================================================
# SegEarth-R2 SET joint training from scratch (Mipha-3B + SigLIP2):
#   Train (LoRA + SEG path + SET modules) → Merge → Parallel test eval → Diagnostic
#
# Purpose:
#   Validate SET-conditioned mask execution under full co-training — NOT frozen A3.
#   Starts from Mipha-3B + mask2former init; does NOT use baseline merged_model.
#
# Code path: segearth+set (SetConditioner on standard batched predictor)
# =============================================================================
set -euo pipefail
...
export WANDB_NAME="${WANDB_NAME:-joint-set-siglip2-from-scratch-50k}"
...
# Output
########################################
OUTPUT_DIR="/root/rivermind-data/huangziyi/reseg/output/set/joint-set-siglip2-from-scratch-80k"
...
EVAL_WANDB_RUN_NAME="${EVAL_WANDB_RUN_NAME:-joint-set-siglip2-from-scratch-80k}"
...
print("[OK] merged model loads; use_set_conditioner=True; category_set_head shape =", shape)
PY
}
...
########################################
echo "========================================"
echo "[0/7] Preflight (SET joint from scratch)"
echo "========================================"
...
########################################
# 1) Train — full joint SET (LoRA + SEG path + SET modules)
########################################
echo "========================================"
echo "[1/7] Train SET joint on LaSeRS (from Mipha-3B + SigLIP2)"
echo "========================================"

{
  echo "=== joint-set-siglip2 from scratch $(date -Iseconds) ==="
...
########################################
echo "========================================"
echo "[4/7] Verify merged model (SET config)"
echo "========================================"
[[ -f "${MERGED_DIR}/config.json" ]] || { echo "[ERROR] merged config missing: ${MERGED_DIR}/config.json"; exit 1; }
verify_merged_set

########################################
# 5) Parallel test eval (5 datasets)
########################################
echo "========================================"
echo "[5/7] Parallel test eval (5 datasets)"
echo "========================================"
