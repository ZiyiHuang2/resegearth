...
class ZeroInitCrossAttentionLayer(CrossAttentionLayer):
    def __init__(self, d_model, nhead, dropout=0.0, activation="relu", normalize_before=False):
        super().__init__(d_model, nhead, dropout, activation, normalize_before)
        nn.init.zeros_(self.multihead_attn.out_proj.weight)
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    def __init__(self, in_channels, num_classes, mask_classification=True, hidden_dim=256, num_queries=100, nheads=8, dim_feedforward=2048, dec_layers=10, pre_norm=False, mask_dim=256, enforce_input_project=False):
        super().__init__()
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size):
        ...
        attn_mask = F.interpolate(outputs_mask.float(), size=attn_mask_target_size, mode="bilinear", align_corners=False).to(mask_embed.dtype)
...
class MultiScaleMaskedTransformerDecoderForOPTPreTrain(nn.Module):
    def __init__(self, in_channels, hidden_dim=256, num_queries=100, nheads=8, dim_feedforward=2048, dec_layers=10, pre_norm=False, mask_dim=256, enforce_input_project=False, seg_norm=False, seg_proj=True, seg_fuse_score=False, use_seg_query=False):
        ...
        self.transformer_self_attention_layers = nn.ModuleList()
        self.transformer_cross_attention_layers = nn.ModuleList()
        self.knowledge_cross_attention_layers = nn.ModuleList()
        self.transformer_ffn_layers = nn.ModuleList()
        self.use_seg_query = use_seg_query
        for _ in range(self.num_layers):
            self.transformer_self_attention_layers.append(SelfAttentionLayer(d_model=hidden_dim, nhead=nheads, dropout=0.0, normalize_before=pre_norm))
            self.transformer_cross_attention_layers.append(CrossAttentionLayer(d_model=hidden_dim, nhead=nheads, dropout=0.0, normalize_before=pre_norm))
            self.knowledge_cross_attention_layers.append(ZeroInitCrossAttentionLayer(d_model=hidden_dim, nhead=nheads, dropout=0.0, normalize_before=pre_norm))
            self.transformer_ffn_layers.append(FFNLayer(d_model=hidden_dim, dim_feedforward=dim_feedforward, dropout=0.0, normalize_before=pre_norm))
    ...
    def forward(self, x, mask_features, mask=None, seg_query=None, SEG_embedding=None, kb_memory=None):
        return self.forward_woconcat(x, mask_features, mask, seg_query, SEG_embedding, kb_memory=kb_memory)

    def forward_woconcat(self, x, mask_features, mask=None, seg_query=None, SEG_embedding=None, kb_memory=None):
        assert len(x) == self.num_feature_levels
        src = []
        pos = []
        size_list = []
        ...
        if self.use_seg_query:
            ...
            query_embed = torch.zeros(self.new_query_embed.weight.shape[0], bs, self.new_query_embed.weight.shape[-1], device=SEG_embedding.device, dtype=SEG_embedding.dtype)
        ...
        if kb_memory is not None:
            kb_memory_seq = kb_memory.permute(1, 0, 2)

        predictions_SEG_class = []
        predictions_mask = []

        SEG_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0], SEG_embedding=SEG_embedding if self.use_seg_query else None)
        ...
        for i in range(self.num_layers):
            ...
            attn_mask[torch.where(attn_mask.sum(-1) == attn_mask.shape[-1])] = False
            output = self.transformer_cross_attention_layers[i](output, src[level_index], memory_mask=attn_mask, memory_key_padding_mask=None, pos=pos[level_index], query_pos=query_embed)
            if kb_memory_seq is not None:
                output = self.knowledge_cross_attention_layers[i](output, kb_memory_seq, memory_mask=None, memory_key_padding_mask=None, pos=None, query_pos=query_embed)
            output = self.transformer_self_attention_layers[i](output, tgt_mask=None, tgt_key_padding_mask=None, query_pos=query_embed)
            output = self.transformer_ffn_layers[i](output)
            SEG_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], SEG_embedding=SEG_embedding if self.use_seg_query else None)
            predictions_SEG_class.append(SEG_class)
            predictions_mask.append(outputs_mask)
        ...
        out = {
            ...
            'pred_masks': predictions_mask[-1],
            'aux_outputs': self._set_aux_loss(predictions_SEG_class, predictions_mask),
        }
        return out
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, SEG_embedding=None):
        ...
        if SEG_embedding is not None:
            decoder_seg_output = self.SEG_proj(decoder_output) if self.seg_proj else decoder_output
            if self.seg_norm:
    ...
    def _set_aux_loss(self, outputs_SEG_class, outputs_seg_masks):
        return [{"pred_SEG_logits": a, "pred_masks": c} for a, c in zip(outputs_SEG_class[:-1], outputs_seg_masks[:-1])]
