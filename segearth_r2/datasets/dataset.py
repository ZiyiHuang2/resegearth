...
from segearth_r2.model.set_conditioner import (
    CategoryLabelStats,
    build_category_set_labels_from_answer,
    load_lasers_category_vocab,
    warn_empty_category_labels_once,
)
...
class LaSeRSDataset(RS_Base_Dataset):
    ...
    def __init__(
        ...
        vocab_path = getattr(data_args, "lasers_category_vocab_path", None)
        self.lasers_category_vocab = load_lasers_category_vocab(vocab_path)
        self.category_label_stats = CategoryLabelStats()
        self._empty_category_warned = False
    ...
    def __getitem__(self, idx):
        ...
        category_labels, category_phrases, matched, unknown = build_category_set_labels_from_answer(
            answer,
            self.lasers_category_vocab,
            stats=self.category_label_stats,
        )
        data_dict['category_set_labels'] = category_labels
        data_dict['category_phrases'] = category_phrases
        if category_labels.sum().item() == 0 and not self._empty_category_warned:
            warn_empty_category_labels_once(category_phrases, unknown)
            self._empty_category_warned = True
...
class DataCollatorForCOCODatasetV2(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'category_set_labels' in instances[0]:
            batch['category_set_labels'] = torch.stack(
                [instance['category_set_labels'] for instance in instances],
                dim=0,
            )
