...
class RRSISDDataset(RS_Base_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data_dict["gt_masks_per_query"] = torch.as_tensor(
            np.expand_dims(masks[:mask_num], axis=1),
            dtype=torch.uint8,
        )
...
class DataCollatorForCOCODatasetV2(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "token_refer_id" in instances[0]:
            batch["token_refer_id"] = [instance["token_refer_id"] for instance in instances]
        ...
        if "mask_num" in instances[0]:
            batch["mask_num"] = [int(instance["mask_num"]) for instance in instances]

        if "query_to_image_index" in instances[0]:
            query_to_image_index = []
            for image_idx, instance in enumerate(instances):
                cur_q2i = instance.get("query_to_image_index", None)
                cur_mask_num = int(instance.get("mask_num", 0))

                if cur_q2i is None:
                    cur_q2i = torch.zeros(cur_mask_num, dtype=torch.long)
                elif not torch.is_tensor(cur_q2i):
                    cur_q2i = torch.tensor(cur_q2i, dtype=torch.long)

                query_to_image_index.append(cur_q2i + image_idx)

            batch["query_to_image_index"] = (
                torch.cat(query_to_image_index, dim=0)
                if len(query_to_image_index) > 0
        ...
        if "gt_masks_per_query" in instances[0]:
            ...
            batch["gt_masks_per_query"] = (
                torch.cat(gt_masks_per_query, dim=0) if len(gt_masks_per_query) > 0 else None
            )

        if "mask_num" in batch and "query_to_image_index" in batch:
            assert sum(batch["mask_num"]) == batch["query_to_image_index"].shape[0], (
                f"sum(mask_num)={sum(batch['mask_num'])} != "
                f"query_to_image_index.shape[0]={batch['query_to_image_index'].shape[0]}"
            )

        if "mask_num" in batch and batch.get("gt_masks_per_query", None) is not None:
            assert sum(batch["mask_num"]) == batch["gt_masks_per_query"].shape[0], (
                f"sum(mask_num)={sum(batch['mask_num'])} != "
                f"gt_masks_per_query.shape[0]={batch['gt_masks_per_query'].shape[0]}"
            )
...
class UnifyDatasetSingleDatasetForBatch(Dataset):
    ...
    def update_dataset_index(self):
        tempt = self.cur_dataset_index + 1
