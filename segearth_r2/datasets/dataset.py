...
from segearth_r2.knowledge.prototype_kb import PrototypeRSKB
...
def preprocess_image(image_path, pad_value=128.0, short_edge_length=1024, max_size=1024):
    ...
    img_padded = np.pad(img_np, padding, mode="constant", constant_values=pad_value)
...
class RS_Base_Dataset(Dataset):
    ...
    def preprocess_llama2(self, sources, tokenizer):
        ...
        input_ids = torch.stack(
            [self.tokenizer_special_tokens(prompt, tokenizer, return_tensors='pt') for prompt in conversations],
            dim=0,
        )
    ...
    def build_prototype_payload(self, instruction: str, category_key: Optional[str] = None):
        if self.prototype_kb is None:
            return {
                "kb_key": "unknown",
                "kb_text": "a generic target object in remote sensing imagery",
                "kb_text_token_id": torch.tensor([], dtype=torch.long),
                "kb_visual_feat": torch.zeros(self.prototype_visual_dim, dtype=torch.float32),
            }
        item = self.prototype_kb.get_item(key=category_key, text=instruction)
        kb_text = item["text_proto"]
        kb_text_token_id = torch.tensor(
            self.tokenizer.encode(kb_text, add_special_tokens=False), dtype=torch.long
        )
        return {
            "kb_key": item["kb_key"],
            "kb_text": kb_text,
            "kb_text_token_id": kb_text_token_id,
            "kb_visual_feat": item["visual_proto"].float(),
        }
...
class RRSISDDataset(RS_Base_Dataset):
    ...
    def __init__(self, base_data_path, tokenizer, data_args, split='train'):
        ...
        self.use_prototype_kb = getattr(data_args, "use_prototype_kb", False)
        self.prototype_visual_dim = getattr(data_args, "prototype_visual_dim", 256)
        self.prototype_kb = PrototypeRSKB(
            kb_path=getattr(data_args, "prototype_kb_path", None),
            unknown_key=getattr(data_args, "prototype_unknown_key", "unknown"),
            visual_dim=self.prototype_visual_dim,
        ) if self.use_prototype_kb else None
        ...
        self.category_dict = {cat["id"]: cat["name"].strip().lower() for cat in data["categories"]}
    ...
    def __getitem__(self, idx):
        ...
        ann = self.ann_dict[ref["ann_id"]]
        cat_id = ann.get("categories_id", None)
        cat_name = self.category_dict.get(cat_id, None)
        ...
        elif self.use_static_kb and self.static_kb is not None and not self.use_prototype_kb:
            aug_instruction = self.static_kb.augment(instruction, max_chars=self.static_kb_max_chars)
        else:
        ...
        refer_instruction = instruction if self.use_prototype_kb else aug_instruction
        token_refer_id = self.preprocess_referring_instruction(refer_instruction)
        ...
        if self.use_prototype_kb:
            proto_payload = self.build_prototype_payload(instruction=instruction, category_key=cat_name)
            data_dict.update(proto_payload)
...
class LaSeRSDataset(RS_Base_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_static_kb and self.static_kb is not None and not self.use_prototype_kb:
            refer_instruction = self.static_kb.augment(instruction, max_chars=self.static_kb_max_chars)
        else:
            refer_instruction = instruction
        token_refer_id = self.preprocess_referring_instruction(refer_instruction)
        ...
        data_dict['mask_num'] = mask_num
        data_dict['raw_instruction'] = instruction
        data_dict['clip_prior_text'] = instruction

        if self.use_prototype_kb:
            category_key = self.prototype_kb.resolve_key_from_text(instruction)
            proto_payload = self.build_prototype_payload(instruction=instruction, category_key=category_key)
            data_dict.update(proto_payload)
...
class DataCollatorForCOCODatasetV2(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(input_ids[0], list):
            ...
            total_input_ids = torch.nn.utils.rnn.pad_sequence(
                total_input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id
            )
            total_input_ids = total_input_ids[:, :self.tokenizer.model_max_length]
            total_labels = [k2 for k1 in labels for k2 in k1]
            total_labels = torch.nn.utils.rnn.pad_sequence(
                total_labels, batch_first=True, padding_value=self.tokenizer.pad_token_id
            )
        ...
        if 'image' in instances[0]:
            if isinstance(instances[0]['file_name'], list):
                ...
                for instance in instances:
                    ...
                    image_clip = [
                        self.clip_image_processor.preprocess(img_clip, return_tensors="pt")["pixel_values"][0]
                        for img_clip in image_clip
                    ]
        ...
        if 'SEG_token_embedding_indices' in instances[0]:
            ...
            SEG_token_embedding_indices = torch.nn.utils.rnn.pad_sequence(
                SEG_token_embedding_indices, batch_first=True, padding_value=0
            )
        ...
        if 'kb_text' in instances[0]:
            batch['kb_text'] = [instance['kb_text'] for instance in instances]

        if 'kb_key' in instances[0]:
            batch['kb_key'] = [instance['kb_key'] for instance in instances]

        if 'kb_text_token_id' in instances[0]:
            batch['kb_text_token_id'] = [instance['kb_text_token_id'] for instance in instances]

        if 'kb_visual_feat' in instances[0]:
            kb_visual_feat = [instance['kb_visual_feat'] for instance in instances]
            batch['kb_visual_feat'] = torch.stack(kb_visual_feat, dim=0)
