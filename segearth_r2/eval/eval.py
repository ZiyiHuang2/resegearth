...
def build_eval_datasets(data_args, tokenizer):
    ...
    if dataset_name == "lasers":
        # Official LaSeRS: test/annotations/*.json (9 benchmarks). Fallback to legacy val/ if exists.
        ...
        split = data_args.split.lower()
        if split not in ["train", "val", "test"]:
            raise ValueError(f"Unsupported LoveDA split: {split}. Must be train / val / test")

        if data_args.local_rank == 0:
            print(f"------ cur benchmark is LoveDA {split} subset -------")

        eval_dataset = LoveDADataset(
            base_data_path=data_args.base_data_path,
            tokenizer=tokenizer,
            data_args=data_args,
            split=split,
