...
def find_linear_layers(model, lora_target_modules=['q_proj', 'v_proj'], train_module_list=[]):
    ...
    for name, module in model.named_modules():
        if isinstance(module, cls) and all([x not in name for x in cur_train_module_list]) and any([x in name for x in lora_target_modules]):
...
def make_unify_datamodule(clip_image_processor, tokenizer, data_args, training_args):
    data_ratio = [int(data_) for data_ in data_args.data_ratio.split('||')]
    datasets = []
    dataset_name = data_args.dataset_name.lower()
    if data_ratio[0] != 0:
        if dataset_name == "rrsisd":
            train_dataset = RRSISDDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="train")
            eval_dataset = RRSISDDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="val")
        elif dataset_name == "lasers":
            train_dataset = LaSeRSDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="train_data.json")
            eval_dataset = LaSeRSDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="val_data.json")
        elif dataset_name == "refsegrs":
            train_dataset = RefSegRSDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="train")
            eval_dataset = RefSegRSDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="val")
        elif dataset_name == "risbench":
            train_dataset = RISBenchDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="train")
            eval_dataset = RISBenchDataset(base_data_path=data_args.base_data_path, tokenizer=tokenizer, data_args=data_args, split="val")
        else:
...
def train():
    ...
    model = SegEarthR2.from_pretrained(model_args.model_name_or_path, mask_decoder_cfg=mask_cfg, add_cross_attn=True, cache_dir=training_args.cache_dir, **bnb_model_from_pretrained_args)
    ...
    model.config.use_prototype_kb = data_args.use_prototype_kb
    model.config.prototype_visual_dim = data_args.prototype_visual_dim
    model.use_prototype_kb = data_args.use_prototype_kb
    model.prototype_visual_dim = data_args.prototype_visual_dim
    ...
    model.resize_token_embeddings(len(tokenizer))
    train_module_list = ["lm_head", "pixel_decoder", "predictor", "SEG_token_projector", "kb_text_projector", "kb_visual_projector"]
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(r=training_args.lora_r, lora_alpha=training_args.lora_alpha, target_modules=lora_target_modules, lora_dropout=training_args.lora_dropout, bias="none", task_type="CAUSAL_LM")
    ...
    if data_args.use_clip_prior:
        ...
        clip_prior_target.initialize_clip_prior(clip_model=clip_prior_target.get_model().get_vision_tower(), tokenizer=tokenizer, clip_model_name_or_path=model_args.vision_tower, map_size=data_args.clip_prior_map_size, alpha=data_args.clip_prior_alpha)
