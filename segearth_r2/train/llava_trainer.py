...
class LLaVATrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if labels is not None:
            ...
            if hasattr(outputs, "loss_dice"):
                loss_dict = {"loss": float(loss.detach().cpu())}
                for name, value in outputs.items():
                    if "loss" in name and name != "loss" and value is not None:
                ...
                for name in ("set_gate_value", "target_count_acc"):
                    value = getattr(outputs, name, None)
                    if value is not None:
                        loss_dict[name] = float(value.item()) if hasattr(value, "item") else float(value)
                if torch.cuda.is_available():
                    loss_dict["gpu_mem_allocated_gb"] = torch.cuda.memory_allocated() / (1024 ** 3)
                    loss_dict["gpu_mem_reserved_gb"] = torch.cuda.memory_reserved() / (1024 ** 3)
    ...
    def log(self, logs):
        if self.args.local_rank in (-1, 0) and torch.cuda.is_available():
            logs.setdefault(
